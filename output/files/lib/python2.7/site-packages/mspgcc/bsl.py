...
class LowLevel:
    ...
    def __init__(self, aTimeout = None, aProlongFactor = None, serialport=None, statusCB=None):
    ...
    def comInit(self, port):
        ...
        if (DEBUG > 1) and (self.statusCB <> None): self.statusCB("* comInit()\n")
        self.seqNo = 0
        self.reqNo = 0
        self.rxPtr = 0
        self.txPtr = 0
        ...
        self.serialport.setLineProperties(databits="8", stopbits="1", parity="Even")
    ...
    def comDone(self):
        ...
        Returns zero if the function is successful."""
        if (DEBUG > 1) and (self.statusCB <> None): self.statusCB("* comDone()")
    ...
    def comRxHeader(self):
        ...
        if (DEBUG > 1) and (self.statusCB <> None): self.statusCB("* comRxHeader()\n")
        ...
        if rxFrame[1] == chr(0) and rxFrame[2] == rxFrame[3]:   #Add. header info. correct?
            ...
            if rxFrame[ord(rxFrame[2])+4] == chr(0xff & checksum) and \
               rxFrame[ord(rxFrame[2])+5] == chr(0xff & (checksum >> 8)): #Checksum correct?
                ...
                if (DEBUG > 2) and (self.statusCB <> None): self.statusCB("* comRxFrame() OK\n")
                return rxFrame
            else:
                if (DEBUG) and (self.statusCB <> None): self.statusCB("  comRxFrame() Checksum wrong\n")
        else:
            if (DEBUG) and (self.statusCB <> None): self.statusCB("  comRxFrame() Header corrupt %r" % rxFrame)
        raise BSLException(self.ERR_COM)            #Frame has errors!

    def comTxHeader(self, txHeader):
        """send header"""
        if (DEBUG > 1) and (self.statusCB <> None): self.statusCB("* txHeader()\n")
    ...
    def comTxRx(self, cmd, dataOut, length):
        ...
        if (DEBUG > 1) and (self.statusCB <> None): self.statusCB("* comTxRx()\n")
        txFrame     = []
        rxHeader    = 0
        rxNum       = 0
        ...
        if self.BSLMemAccessWarning and accessAddr < self.BSL_CRITICAL_ADDR:
            if (self.statusCB <> None): self.statusCB("WARNING: This command might change data at address %04x or %04x!\n" % (accessAddr, accessAddr + 1))
        ...
        for c in txFrame:
            ...
            if (DEBUG > 3) and (self.statusCB <> None): self.statusCB("\ttx %02x" % ord(c))
            #if self.serialport.inWaiting(): break  #abort when BSL replies, probably NAK
        else:
            if (DEBUG > 1) and (self.statusCB <> None): self.statusCB( "  comTxRx() transmit OK\n")
        ...
        if self.ignoreAnswer:
            ...
            if (DEBUG > 1) and (self.statusCB <> None): self.statusCB("  comTxRx() rxHeader=0x%02x, rxNum=%d, seqNo=%d, reqNo=%s\n" % (rxHeader, rxNum, self.seqNo, self.reqNo))
            if rxHeader == self.DATA_ACK:               #acknowledge/OK
                if (DEBUG > 2) and (self.statusCB <> None): self.statusCB("  comTxRx() DATA_ACK\n")
                if rxNum == self.reqNo:
                    self.seqNo = self.reqNo
                    if (DEBUG > 2) and (self.statusCB <> None): self.statusCB("* comTxRx() DATA_ACK OK\n")
                ...
                if (DEBUG > 2) and (self.statusCB <> None): self.statusCB("* comTxRx() DATA_NAK\n")
                raise BSLException(self.ERR_RX_NAK)
            elif rxHeader == self.DATA_FRAME:           #receive data
                if (DEBUG > 2) and (self.statusCB <> None): self.statusCB("* comTxRx() DATA_FRAME\n")
                if rxNum == self.reqNo:
                    rxFrame = self.comRxFrame(rxNum)
                    return rxFrame
                raise BSLException(self.ERR_FRAME_NUMBER)
            elif rxHeader == self.CMD_FAILED:           #Frame ok, but command failed.
                if (DEBUG > 2) and (self.statusCB <> None): self.statusCB("*  comTxRx() CMD_FAILED\n")
    ...
    def bslReset(self, invokeBSL=0):
        ...
        if (DEBUG > 1) and (self.statusCB <> None): self.statusCB("* bslReset(invokeBSL=%s)\n" % invokeBSL)
    ...
    def bslSync(self,wait=0):
        ...
        if (DEBUG > 1) and (self.statusCB <> None): self.statusCB("* bslSync(wait=%d)\n" % wait)
        if self.ignoreAnswer:
            ...
            while wait or loopcnt:
                ...
                if c == chr(self.DATA_ACK):             #ACk
                    if (DEBUG > 1) and (self.statusCB <> None): self.statusCB("  bslSync() OK\n")
                    return                              #Sync. successful
                elif not c:                             #timeout
                    if DEBUG > 1:
                        if loopcnt:
                            if (self.statusCB <> None): self.statusCB("  bslSync() timeout, retry ...\n")
                        else:
                            if (self.statusCB <> None): self.statusCB("  bslSync() timeout\n")
                else:                                   #garbage
                    if (DEBUG > 1) and (self.statusCB <> None): self.statusCB("  bslSync() failed (0x%02x), retry ...\n" % ord(c))
    ...
    def bslTxRx(self, cmd, addr, length = 0, blkout = None, wait=0):
        ...
        """
        if (DEBUG > 1) and (self.statusCB <> None): self.statusCB("* bslTxRx()\n")
...
class BootStrapLoader(LowLevel):
    ...
    def preparePatch(self):
        ...
        if (DEBUG > 1) and (self.statusCB <> None): self.statusCB("* preparePatch()\n")
    ...
    def postPatch(self):
        ...
        if (DEBUG > 1) and (self.statusCB <> None): self.statusCB("* postPatch()\n")
        if self.patchLoaded:
    ...
    def verifyBlk(self, addr, blkout, action):
        ...
        if (DEBUG > 1) and (self.statusCB <> None): self.statusCB("* verifyBlk()\n")

        if action & self.ACTION_VERIFY or action & self.ACTION_ERASE_CHECK:
            if (DEBUG) and (self.statusCB <> None): self.statusCB("  Check starting at 0x%04x, %d bytes ... \n" % (addr, len(blkout)))
            ...
            for i in range(len(blkout)):
                if action & self.ACTION_VERIFY:
                    ...
                    if blkin[i] != blkout[i]:
                        if (self.statusCB <> None): self.statusCB("Verification failed at 0x%04x (0x%02x, 0x%02x)\n" % (addr+i, ord(blkin[i]), ord(blkout[i])))
                    ...
                    if blkin[i] != chr(0xff):
                        if (self.statusCB <> None): self.statusCB("Erase Check failed at 0x%04x (0x%02x)\n" % (addr+i, ord(blkin[i])))
    ...
    def programBlk(self, addr, blkout, action):
        ...
        if (DEBUG > 1) and (self.statusCB <> None): self.statusCB("* programBlk()\n")

        #Check, if specified range is erased
        self.verifyBlk(addr, blkout, action & self.ACTION_ERASE_CHECK)

        if action & self.ACTION_PROGRAM:
            if (DEBUG > 1) and (self.statusCB <> None): self.statusCB("  Program starting at 0x%04x, %i bytes ...\n" % (addr, len(blkout)))
            self.preparePatch()
            #Program block
    ...
    def progess_update(self, count, total):
        ...
        if (self.statusCB <> None): self.statusCB("\r%d%%" % (100*count/total))
    ...
    def programData(self, segments, action):
        ...
        if (DEBUG > 1) and (self.statusCB <> None): self.statusCB("* programData()\n")
    ...
    def uploadData(self, startaddress, size, wait=0):
        """Upload a datablock"""
        if (DEBUG > 1) and (self.statusCB <> None): self.statusCB("* uploadData()\n")
        data = ''
        pstart = 0
        while pstart<size:
            length = self.maxData
    ...
    def txPasswd(self, passwd=None, wait=0):
        ...
        if (DEBUG > 1) and (self.statusCB <> None): self.statusCB("* txPassword(%r)\n" % passwd)
        if passwd is None:
            #Send "standard" password to get access to protected functions.
            if (self.statusCB <> None): self.statusCB("Transmit default password ...\n")
            ...
            if (self.statusCB <> None): self.statusCB('Transmit password ...\n')
        #send the password
    ...
    def actionMassErase(self, sendpasswd=1, bslreset=1):
        """Erase the flash memory completely (with mass erase command)"""
        if (self.statusCB <> None): self.statusCB("Mass Erase...\n")
        if bslreset:
            self.bslReset(1)                            #Invoke the boot loader.
        for i in range(self.meraseCycles):
            if i == 1: 
                if (self.statusCB <> None): self.statusCB("Additional Mass Erase Cycles...\n")
    ...
    def actionMainErase(self):
        ...
        if (self.statusCB <> None): self.statusCB("Main Erase...\n")
    ...
    def makeActionSegmentErase(self, address):
        ...
        class SegmentEraser:
            ...
            def __call__(inner_self):
                if (self.statusCB <> None): self.statusCB("Erase Segment @ 0x%04x...\n" % inner_self.address)
    ...
    def actionStartBSL(self, usepatch=1, adjsp=1, replacementBSL=None, forceBSL=0, mayuseBSL=0, speed=None, bslreset=1, sendpasswd=1):
        """Start BSL, download patch if desired and needed, adjust SP if desired, download
        replacement BSL, change baudrate."""
        if (self.statusCB <> None): self.statusCB("Invoking BSL...\n")
        if bslreset:
            self.bslReset(1)                        #Invoke the boot loader.
        if sendpasswd:
        ...
        if (DEBUG > 1) and (self.statusCB <> None): self.statusCB("Reading BSL version ...\n")
        ...
        if self.cpu is None:                        #cpy type forced?
            if deviceids.has_key(dev_id):
                ...
                if (DEBUG) and (self.statusCB <> None): self.statusCB("Autodetect successful: %04x -> %s\n" % (dev_id, self.cpu))
            else:
                if (self.statusCB <> None): self.statusCB("Autodetect failed! Unknown ID: %04x. Trying to continue anyway.\n" % dev_id)
                self.cpu = F1x                      #assume something and try anyway..

        if (self.statusCB <> None): self.statusCB("Current bootstrap loader version: %x.%x (Device ID: %04x)\n" % (bslVerHi, bslVerLo, dev_id))
        ...
        if self.bslVer <= 0x0130 and adjsp:
            ...
            if (self.statusCB <> None): self.statusCB("Adjust SP. Load PC with 0x0C22 ...\n")
            self.bslTxRx(self.BSL_LOADPC,           #Command: Load PC
        ...
        if ((mayuseBSL and speed and self.bslVer < 0x0150) or forceBSL) and replacementBSL is None:
            ...
            if self.cpu == F4x:
                if (DEBUG) and (self.statusCB <> None): self.statusCB("Using built in BSL replacement for F4x devices\n")
                replacementBSL.loadTIText(cStringIO.StringIO(F4X_BSL))  #parse embedded BSL
            else:
                if (DEBUG) and (self.statusCB <> None): self.statusCB("Using built in BSL replacement for F1x devices\n")
        ...
        if (DEBUG) and (self.statusCB <> None): self.statusCB("Current bootstrap loader version: 0x%04x\n" % (self.bslVer,))

        #now apply workarounds or patches if BSL in use requires that
        if self.bslVer <= 0x0110:                   #check if patch is needed
            if usepatch:                            #test if patch is desired
                if (self.statusCB <> None): self.statusCB("Patch for flash programming required!\n")
                self.patchRequired = 1

                if (self.statusCB <> None): self.statusCB("Load and verify patch ...\n")
                ...
                if (DEBUG) and (self.statusCB <> None): self.statusCB("Device needs patch, but not applied (usepatch is false).\n")    #message if not patched
    ...
    def actionDownloadBSL(self, bslsegments):
        ...
        if (self.statusCB <> None): self.statusCB("Load new BSL into RAM...\n")
        self.programData(bslsegments, self.ACTION_PROGRAM)
        if (self.statusCB <> None): self.statusCB("Verify new BSL...\n")
        ...
        if (self.statusCB <> None): self.statusCB("Starting new BSL at 0x%04x...\n" % startaddr)
        self.bslTxRx(self.BSL_LOADPC,  #Command: Load PC
    ...
    def actionEraseCheck(self):
        ...
        if (self.statusCB <> None): self.statusCB("Erase Check by file ...\n")
    ...
    def actionProgram(self):
        ...
        if self.data is not None:
            if (self.statusCB <> None): self.statusCB("Program ...\n")
            self.programData(self.data, self.ACTION_PROGRAM)
            if (self.statusCB <> None): self.statusCB("%i bytes programmed.\n" % self.byteCtr)
        else:
    ...
    def actionVerify(self):
        ...
        if self.data is not None:
            if (self.statusCB <> None): self.statusCB("Verify ...\n")
    ...
    def actionReset(self):
        ...
        if (self.statusCB <> None): self.statusCB("Reset device ...\n")
        self.bslReset(0) #only reset

    def actionRun(self, address=0x220):
        """Start program at specified address"""
        if (self.statusCB <> None): self.statusCB("Load PC with 0x%04x ...\n" % address)
        self.bslTxRx(self.BSL_LOADPC, #Command: Load PC
    ...
    def actionChangeBaudrate(self, baudrate=9600):
        ...
        if (self.statusCB <> None): self.statusCB("Changing baudrate to %d ...\n" % baudrate)
        if baudrate > 38400:
            if (self.statusCB <> None): self.statusCB("Note: The selected baudrate is not TI standard! They will not work with TI's BSLs.\n")
