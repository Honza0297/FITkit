...
static int device_open(struct usb_device *dev)
{
  ...
  snprintf(filename, sizeof(filename) - 1, "%.50s/%.50s/%.50s",
  ...
  if (fd < 0) {
    ...
    if (fd < 0)
      USB_ERROR_STR(-errno, "failed to open %.50s: %.50s",
...
int usb_os_close(usb_dev_handle *dev)
{
  ...
  if (close(dev->fd) == -1)
    ...
    USB_ERROR_STR(0, "tried to close device fd %d: %.50s", dev->fd,
	strerror(errno));
...
int usb_set_configuration(usb_dev_handle *dev, int configuration)
{
  ...
  if (ret < 0)
    USB_ERROR_STR(-errno, "could not set config %d: %.50s", configuration,
	strerror(errno));
...
int usb_claim_interface(usb_dev_handle *dev, int interface)
{
  ...
  if (ret < 0) {
    if (errno == EBUSY && usb_debug > 0)
      fprintf(stderr, "Check that you have permissions to write to %.50s/%.50s and, if you don't, that you set up hotplug (http://linux-hotplug.sourceforge.net/) correctly.\n", dev->bus->dirname, dev->device->filename);

    USB_ERROR_STR(-errno, "could not claim interface %d: %.50s", interface,
	strerror(errno));
  }
...
int usb_release_interface(usb_dev_handle *dev, int interface)
{
  ...
  if (ret < 0)
    USB_ERROR_STR(-errno, "could not release intf %d: %.50s", interface,
    	strerror(errno));
...
int usb_set_altinterface(usb_dev_handle *dev, int alternate)
{
  ...
  if (ret < 0)
    USB_ERROR_STR(-errno, "could not set alt intf %d/%d: %.50s",
	...
	int value, int index, char *bytes, int size, int timeout)
{
  ...
  if (ret < 0)
    USB_ERROR_STR(-errno, "error sending control message: %.50s", strerror(errno));
	...
	char *bytes, int size, int timeout)
{
  ...
  do {
    ...
    if (ret < 0) {
      USB_ERROR_STR(-errno, "error submitting URB: %.50s", strerror(errno));
      return ret;
    }
...
restart:
    ...
    if (ret < 0 && !urb.usercontext && errno != EAGAIN)
      USB_ERROR_STR(-errno, "error reaping URB: %.50s", strerror(errno));
  ...
  if (ret < 0 && !urb.usercontext) {
    ...
    if (ret < 0 && errno != EINVAL && usb_debug >= 1)
      fprintf(stderr, "error discarding URB: %.50s", strerror(errno));
...
int usb_os_find_busses(struct usb_bus **busses)
{
  ...
  if (!dir)
    USB_ERROR_STR(-errno, "couldn't opendir(%.50s): %.50s", usb_path,
	strerror(errno));
  ...
  while ((entry = readdir(dir)) != NULL) {
    ...
    if (!strchr("0123456789", entry->d_name[strlen(entry->d_name) - 1])) {
      if (usb_debug >= 2)
        fprintf(stderr, "usb_os_find_busses: Skipping non bus directory %.50s\n",
		entry->d_name);
      continue;
    }
    ...
    if (usb_debug >= 2)
       fprintf(stderr, "usb_os_find_busses: Found %.50s\n", bus->dirname);
  }
...
int usb_os_find_devices(struct usb_bus *bus, struct usb_device **devices)
{
  ...
  snprintf(dirpath, PATH_MAX, "%.50s/%.50s", usb_path, bus->dirname);

  dir = opendir(dirpath);
  if (!dir)
    USB_ERROR_STR(-errno, "couldn't opendir(%.50s): %.50s", dirpath,
	strerror(errno));
  ...
  while ((entry = readdir(dir)) != NULL) {
    ...
    snprintf(filename, sizeof(filename) - 1, "%.50s/%.50s", dirpath, entry->d_name);
    fd = open(filename, O_RDWR);
    if (fd < 0) {
      fd = open(filename, O_RDONLY);
      if (fd < 0) {
        if (usb_debug >= 2)
          fprintf(stderr, "usb_os_find_devices: Couldn't open %.50s\n",
                  filename);
    ...
    if (usb_debug >= 2)
      fprintf(stderr, "usb_os_find_devices: Found %.50s on %.50s\n",
  ...
  for (dev = bus->devices; dev; dev = dev->next) {
    ...
    if (ret < 0) {
      ...
      if (errno != ENOSYS && usb_debug > 1)
        fprintf(stderr, "error obtaining child information: %.50s\n",
		strerror(errno));
...
void usb_os_init(void)
{
  ...
  if (usb_debug) {
    if (usb_path[0])
      fprintf(stderr, "usb_os_init: Found USB VFS at %.50s\n", usb_path);
    else
...
int usb_resetep(usb_dev_handle *dev, unsigned int ep)
{
  ...
  if (ret)
    USB_ERROR_STR(-errno, "could not reset ep %d: %.50s", ep,
    	strerror(errno));
...
int usb_clear_halt(usb_dev_handle *dev, unsigned int ep)
{
  ...
  if (ret)
    USB_ERROR_STR(-errno, "could not clear/halt ep %d: %.50s", ep,
    	strerror(errno));
...
int usb_reset(usb_dev_handle *dev)
{
  ...
  if (ret)
     USB_ERROR_STR(-errno, "could not reset: %.50s", strerror(errno));
	...
	unsigned int namelen)
{
  ...
  if (ret)
    USB_ERROR_STR(-errno, "could not get bound driver: %.50s", strerror(errno));
...
int usb_detach_kernel_driver_np(usb_dev_handle *dev, int interface)
{
  ...
  if (ret)
    USB_ERROR_STR(-errno, "could not detach kernel driver from interface %d: %.50s",
