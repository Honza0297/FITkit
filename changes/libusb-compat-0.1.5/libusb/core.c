...
API_EXPORTED int usb_find_devices(void)
{
	...
	for (bus = usb_busses; bus; bus = bus->next) {
		...
		printf("in usb_find_devices. found device is: %d\n", r);
		if (r < 0) {
			libusb_free_device_list(dev_list, 1);
			return r;
		}
		...
		while (dev) {
			...
			if (!found) {
				...
				printf("device %d.%d removed", dev->bus->location, dev->devnum);
		...
		while (dev) {
			...
			if (r < 0) {
				printf("couldn't initialize device %d.%d (error %d)", dev->bus->location, dev->devnum, r);
	...
	libusb_free_device_list(dev_list, 1);
	printf("in usb_find_devices: return value is: %d\n", changes);
	return changes;
}
