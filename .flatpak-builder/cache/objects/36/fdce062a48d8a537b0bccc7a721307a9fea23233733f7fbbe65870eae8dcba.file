...
/** Allocate new packet.
  ...
  * \return new packet
  */
URPC_EXPORT Packet* pkt_new(uint32_t size, uint8_t op);

/** Free allocated packet.
  * Use only with dynamically allocated packets.
  * \param pkt freed packet
  */
URPC_EXPORT void pkt_free(Packet* pkt);
...
/** Reserve given size.
  ...
  */
URPC_EXPORT int pkt_reserve(Packet* pkt, uint32_t size);

/** Initialize packet.
  * \param pkt initialized packet
  * \param op packet opcode
  */
URPC_EXPORT void pkt_init(Packet* pkt, uint8_t op);

/** Return shared packet.
  * \return ptr to shared packet
  */
URPC_EXPORT Packet* pkt_shared();

/** Claim shared packet buffer.
  * \return ptr to shared packet
  */
URPC_EXPORT Packet* pkt_claim();

/** Release shared packet buffer.
  */
URPC_EXPORT void pkt_release();
...
/** Append parameter to packet.
  ...
  */
URPC_EXPORT int pkt_append(Packet* pkt, uint8_t type, uint16_t len, const void* val);

/** Append numeric value. */
URPC_EXPORT int pkt_addnumeric(Packet* pkt, uint8_t type, uint16_t len, int32_t val);
...
/** Receive packet.
  ...
  */
URPC_EXPORT uint32_t pkt_recv(int fd, Packet* dst);
...
/** Send packet.
  ...
  */
URPC_EXPORT int pkt_send(Packet* pkt, int fd);
...
/** Set iterator to first packet payload.
  ...
  */
URPC_EXPORT void* pkt_begin(Packet* pkt, Iterator* it);


/** Return true on iterator end.
  * \return true if iterator is at lastpos + 1
  */
URPC_EXPORT int iter_end(Iterator*);

/** Shift to next item.
  * \return next item ptr or NULL on error
  */
URPC_EXPORT void* iter_next(Iterator* it);

/** Return current item value and shift to next item.
  * \return current item value
  */
URPC_EXPORT void* iter_nextval(Iterator* it);

/** Step in current structure item.
  * \return next item ptr or NULL on error
  */
URPC_EXPORT void* iter_enter(Iterator* it);
